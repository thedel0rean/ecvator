...
{
    CreateDirectoryA(skCrypt("C:\\ecvator\\"), NULL);
    path /= std::string(skCrypt("C:\\ecvator\\Configs"));
    listConfigs();
    misc.clanTag[0] = '\0';
    misc.name[0] = '\0';
...
static void from_json(const json& j, Config::RagebotGlobal& rg)
{
    ...
    read(j, "Resolver", rg.resolver);
    read(j, "Resolver methode", rg.resolver_methode);
    read(j, "Resolver Roll", rg.resolverRoll);

    read(j, "DMP", rg.disableMultipointIfLowFPS);
}
...
static void from_json(const json& j, Config::Menu& m)
{
    ...
    read<value_t::object>(j, "ACCENT WBG", m.accentColorWindowsBG);
    read<value_t::object>(j, "ACCENT CBG", m.accentColorChildBG);
    read<value_t::object>(j, "ACCENT PBG", m.accentColorPopupBG);

    read(j, "OUTLINE", m.outline_menu);
...
void Config::load(const char8_t* name, bool incremental) noexcept
{
    ...
    read(j, "Resolver Key", resolverKey);
    ...
    read(j, "Force spoint", forceSPoints);
...
static void to_json(json& j, const Color3& o, const Color3& dummy = {})
{
    ...
    WRITE("Accent Color Windows BG", accentColorWindowsBG);
    WRITE("Accent Color Child BG", accentColorChildBG);
    WRITE("Accent Color Popup BG", accentColorPopupBG);
...
static void to_json(json& j, const Config::Menu& o)
{
    ...
    WRITE("ACCENT WBG", accentColorWindowsBG);
    WRITE("ACCENT CBG", accentColorChildBG);
    WRITE("ACCENT PBG", accentColorPopupBG);

    WRITE("OUTLINE", outline_menu);
...
static void to_json(json& j, const Config::RagebotGlobal& o, const Config::RagebotGlobal& dummy = {})
{
    ...
    WRITE("Resolver methode", resolver_methode);
    WRITE("ResolverRoll", resolverRoll);

    WRITE("DMPLFPS", disableMultipointIfLowFPS);
}
static void to_json(json& j, const Config::Ragebot& o, const Config::Ragebot& dummy = {})
{
    WRITE("OV CONFIG", enabled);
    WRITE("ASS", autoStop);
    WRITE("ASM", autostop_mode);
    WRITE("HB", hitboxes);
    WRITE("MHB", multi_hitboxes);
    WRITE("HC", hitChance);
    WRITE("DTHC", DThitChance);
    WRITE("HCO", OvrHitChance);
    WRITE("HCOB", hcov);
    WRITE("MH", multiPointHead);
    WRITE("MB", multiPointBody);
    WRITE("MS", multiPointScale);
    WRITE("MD", minDamage);
    WRITE("MDO", minDamageOverride);
    WRITE("BS", betweenShots);

    WRITE("PBA", preferBodyAim);
    WRITE("PSP", preferSafePoints);

    WRITE("DL", delayShot);
    WRITE("FB", force_baim_onpeek);
}
...
void Config::save(size_t id) const noexcept
{
    ...
    if (std::ofstream out{ path / (const char8_t*)configs[id].c_str() }; out.good()) {
        ...
        to_json(j["Resolver Key"], ragebotKey, KeyBind::NONE);
