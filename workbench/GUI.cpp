...
#include "../workbench/SDK/Surface.h"
...
inline constexpr float animationLength() { return 0.55f; }
float toggleAnimationEnd = 0.0f;

float getTransparency() noexcept
{
    return std::clamp(gui->isOpen() ? toggleAnimationEnd : 1.0f - toggleAnimationEnd, 0.0f, 1.0f);
}
...
GUI::GUI() noexcept
{
    ...
    if (PWSTR pathToFonts; SUCCEEDED(SHGetKnownFolderPath(FOLDERID_Fonts, 0, nullptr, &pathToFonts)))
    {
        ...
        fonts.tab_ico = io.Fonts->AddFontFromMemoryTTF((void*)main_icon, sizeof(main_icon), 14.0f, &cfg, io.Fonts->GetGlyphRangesCyrillic());
        ...
        fonts.smallfonts = io.Fonts->AddFontFromMemoryTTF((void*)smallestcock, sizeof(smallestcock), 13.0f, &cfg1, io.Fonts->GetGlyphRangesCyrillic());
        fonts.weaponIcons = io.Fonts->AddFontFromMemoryTTF((void*)weaponIcon, sizeof(weaponIcon), 14.0f, &cfg1, io.Fonts->GetGlyphRangesCyrillic());
...
void GUI::render() noexcept
{
    ...
    // Style->Colors[ImGuiCol_Border] = ImColor(Helpers::calculateColor(config->menu.accentColor));
...
bool GUI::tab(const char* label, bool selected)
{
    ...
    ImColor col{};
    if (selected)
        col = ImColor(Helpers::calculateColor(config->menu.accentColor));
    else
        col = ImColor(155, 160, 156);

    ImGui::PushFont(fonts.verdana);
    window->DrawList->AddText(ImVec2((rect.Min.x + rect.Max.x + 1) / 2.f - (label_size.x / 2.f), (rect.Min.y + rect.Max.y) / 2.f - (label_size.y / 2.f)), ImColor(15, 15, 15), label);
    window->DrawList->AddText(ImVec2((rect.Min.x + rect.Max.x) / 2.f - (label_size.x / 2.f), (rect.Min.y + rect.Max.y) / 2.f - (label_size.y / 2.f)), col, label);
    ImGui::PopFont();
...
void GUI::render_gui_menu() noexcept
{
    ImGuiStyle* Style = &ImGui::GetStyle();
    ...
    Style->Colors[ImGuiCol_TextDisabled] = ImColor(200, 200, 200);
    ...
    Style->Colors[ImGuiCol_PopupBg] = ImColor(15, 15, 15);
    Style->Colors[ImGuiCol_WindowBg] = ImColor(15, 15, 15, 225);
    ...
    ImGui::SetNextWindowSize(ImVec2(750, 600), ImGuiCond_Once);
    ImGui::Begin(skCrypt("##ecvator"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoCollapse);
    {
        static auto selected = 0;

        ImGui::BeginChild(skCrypt("##tabs"), ImVec2{ 150, 0 }, true);
        {
            //ImGui::SetCursorPos(ImVec2{ 30, 10 });
            //ImGui::Image(myImage, ImVec2(60.f, 60.f));

            ImGui::SetCursorPos(ImVec2{ 45, 30 });
            ImGui::PushFont(fonts.tahoma24);
            ImGui::TextColored(ImColor(235, 235, 235), "ecvator");
            ImGui::PopFont();

            ImGui::SetCursorPos(ImVec2{ 57, 55 });
            ImGui::PushFont(fonts.tahoma16);
            ImGui::TextColored(ImColor(Helpers::calculateColor(config->menu.accentColor)), "alpha");
            ImGui::PopFont();

            ImGui::SetCursorPos({ 40, 140 });
            ImGui::BeginGroup();
            {
                if (tab("Aimbot", 0 == selected)) selected = 0; // ImGui::SameLine(0.f, 21.f);
                if (tab("Anti-aim", 1 == selected)) selected = 1; // ImGui::SameLine(0.f, 21.f);
                if (tab("Visuals", 2 == selected)) selected = 2; // ImGui::SameLine(0.f, 21.f);
                if (tab("Players", 3 == selected)) selected = 3; // ImGui::SameLine(0.f, 21.f);
                if (tab("Misc", 4 == selected)) selected = 4;
                if (tab("Skins", 5 == selected)) selected = 5;
                if (tab("Configs", 6 == selected)) selected = 6;
                // if (tab("Scripts", 7 == selected)) selected = 7;
                if (tab("Interface", 8 == selected)) selected = 8;
            }
            ImGui::EndGroup();
        }
        ImGui::EndChild();
        ...
        if (selected == 0)
        {
            ...
            static bool multipoint_hitboxes[]{ false, false, false, false, false };
            static const char* multipoint_hitbox[]{ "Head", "Chest", "Body", "Arms", "Legs" };
            static std::string preview_value = "";

            static bool autoStop_modes[]{ false, false, false };
            static const char* autoStop_mode[]{ "Early", "Beetween shoot", "Force accuracy" };
            static std::string prev_value = "";

            bool once = false;

            ImGui::SetCursorPos({ 160, 10 });
            {
                ImGui::SameLine();
                ImGui::BeginChild(skCrypt("##rageweap"), ImVec2{ 568, 52 }, true);
                {
                    ...
                    ImGui::BeginGroup();
                    {
                        ...
                        if (ImGui::Button(skCrypt("General"), ImVec2{ 70,30 })) RcurrentCategory = 0;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        ImGui::PushFont(gui->weaponIcons());
                        if (RcurrentCategory == 1) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("D"), ImVec2{ 50,30 })) RcurrentCategory = 1;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 2) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("A"), ImVec2{ 50,30 })) RcurrentCategory = 2;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 3) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("J"), ImVec2{ 50,30 })) RcurrentCategory = 3;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        //if (RcurrentCategory == 4) ActiveTab1(); else InactiveTab1();
                        //if (ImGui::Button(skCrypt("L"), ImVec2{ 50,30 })) RcurrentCategory = 4;
                        //ImGui::PopStyleColor(4);
                        //ImGui::SameLine();
                        if (RcurrentCategory == 5) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("W"), ImVec2{ 50,30 })) RcurrentCategory = 5;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 6) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("a"), ImVec2{ 50,30 })) RcurrentCategory = 6;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 7) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("Z"), ImVec2{ 50,30 })) RcurrentCategory = 7;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 8) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("Y"), ImVec2{ 50,30 })) RcurrentCategory = 8;
                        ImGui::PopStyleColor(4);
                        ImGui::SameLine();
                        if (RcurrentCategory == 9) ActiveTab1(); else InactiveTab1();
                        if (ImGui::Button(skCrypt("h"), ImVec2{ 50,30 })) RcurrentCategory = 9;
                        ImGui::PopStyleColor(4);
                        ImGui::PopFont();                   
                    }
                    ImGui::EndGroup();
            ...
            ImGui::SetCursorPos({ 170, 75 });
            {
                // ImGui::SameLine();
                ImGui::BeginChild(skCrypt("##rage"), ImVec2{ 280, 0 }, true);
                {
                    ImGui::SetCursorPos({ 10, 10 });
                    ImGui::BeginGroup();
                    {
                        ImGui::PushFont(fonts.tahoma16);
                        ImGui::TextColored(ImColor(225, 225, 225), skCrypt("General"));
                        ImGui::PopFont();

                        ImGui::Spacing();

                        ImGui::PushItemWidth(245.f);
                        ImGui::Checkbox(skCrypt("Enabled"), &config->ragebot.enabled);
                        ImGui::SameLine();
                        ImGui::PushID(skCrypt("smth Key"));
                        ImGui::hotkey2(skCrypt(""), config->ragebotKey);
                        ImGui::PopID();

                        if (config->ragebot.enabled)
                        {
                            config->ragebot.knifebot = true;
                        ...
                        if (ImGui::BeginPopup(" "))
                        {
                            ImGui::Checkbox(skCrypt("Resolver"), &config->ragebot.resolver);
                        ...
                        ImGui::Checkbox(skCrypt("Silent aim"), &config->ragebot.silent);
                        ImGui::Checkbox(skCrypt("Automatic shot"), &config->ragebot.autoShot);
                        ImGui::Checkbox(skCrypt("Automatic scope"), &config->ragebot.autoScope);
                        ...
                        ImGui::PushID("Automatic peek Key");
                        ImGui::hotkey2("", config->misc.autoPeekKey);
                        ImGui::PopID();

                        ImGui::PushID(skCrypt("Double-tap"));
                        ...
                        if (ImGui::BeginPopup(" "))
                        {
                            ImGui::Checkbox(skCrypt("Fast recharged"), &config->tickbase.fast_recharge);
                            ImGui::Checkbox(skCrypt("Defensive double-tap"), &config->tickbase.defensive_dt);
                            ImGui::EndPopup();
                        }
                        ImGui::PopID();

                        ImGui::PushID("Hide-shots");
                        ImGui::hotkey2(skCrypt("Hide-shots"), config->tickbase.hideshots);
                        ImGui::PopID();

                        ImGui::SliderFloat(skCrypt("Field of view"), &config->ragebot.fov, 1.f, 180.f, "%.1f");
                        ImGui::Checkbox(skCrypt("Ping spike"), &config->backtrack.fakeLatency);
                        ...
                        if (ImGui::BeginPopup(" "))
                        {
                            ...
                            ImGui::SliderFloat(skCrypt("Duration"), &config->visuals.onHitHitbox.duration, 0.5f, 15.f, "%.1f");
                            ImGui::EndPopup();
                        }
                        ImGui::PopID();
                ...
                ImGui::EndChild();
                ImGui::SameLine();
                ImGui::BeginChild(skCrypt("##rage2"), ImVec2{ 280, 0 }, true);
                {
                    ImGui::SetCursorPos({ 10, 10 });
                    ImGui::BeginGroup();
                    {
                        ImGui::PushFont(fonts.tahoma16);
                        ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Selection"));
                        ImGui::PopFont();
                        ...
                        if (ImGui::BeginCombo(skCrypt("Hitboxes"), previewvalue.c_str()))
                        {
                            previewvalue = "";
                        ...
                        for (size_t i = 0; i < ARRAYSIZE(autoStop_modes); i++)
                        {
                            autoStop_modes[i] = (config->rageBot[RcurrentCategory].autostop_mode & 1 << i) == 1 << i;
                        }

                        if (ImGui::BeginCombo("Automatic stop mode", prev_value.c_str()))
                        {
                            prev_value = "";
                            for (size_t i = 0; i < ARRAYSIZE(autoStop_modes); i++)
                            {
                                ImGui::Selectable(autoStop_mode[i], &autoStop_modes[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
                            }
                            ImGui::EndCombo();
                        }
                        ...
                        for (size_t i = 0; i < ARRAYSIZE(autoStop_modes); i++)
                        {
                            ...
                            if (autoStop_modes[i])
                            {
                                prev_value += prev_value.size() ? std::string(", ") + autoStop_mode[i] : autoStop_mode[i];
                                config->rageBot[RcurrentCategory].autostop_mode |= 1 << i;
                            }
                            else
                            {
                                config->rageBot[RcurrentCategory].autostop_mode &= ~(1 << i);
                        ...
                        //for (size_t i = 0; i < ARRAYSIZE(multipoint_hitboxes); i++)
                        //{
                        //    multipoint_hitboxes[i] = (config->rageBot[RcurrentCategory].multi_hitboxes & 1 << i) == 1 << i;
                        //}

                        //if (ImGui::BeginCombo("Multi-point hitboxes", preview_value.c_str()))
                        //{
                        //    preview_value = "";
                        //    for (size_t i = 0; i < ARRAYSIZE(multipoint_hitboxes); i++)
                        //    {
                        //        ImGui::Selectable(multipoint_hitbox[i], &multipoint_hitboxes[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
                        //    }
                        //    ImGui::EndCombo();
                        //}
                        ...
                        //    if (multipoint_hitboxes[i])
                        //    {
                        //        preview_value += preview_value.size() ? std::string(", ") + multipoint_hitbox[i] : multipoint_hitbox[i];
                        //        config->rageBot[RcurrentCategory].multi_hitboxes |= 1 << i;
                        //    }
                        //    else
                        //    {
                        //        config->rageBot[RcurrentCategory].multi_hitboxes &= ~(1 << i);
                        ...
                        ImGui::SliderInt(skCrypt("Hit-chance"), &config->rageBot[RcurrentCategory].hitChance, 0, 100, "%d");
                        ImGui::SliderInt(skCrypt("Double-tap hitchance"), &config->rageBot[RcurrentCategory].DThitChance, 0, 100, "%d");
                        ImGui::SliderInt(skCrypt("Multi-point scale"), &config->rageBot[RcurrentCategory].multiPointScale, 0, 100, "%d");
                        ImGui::SliderInt(skCrypt("Min.Damage"), &config->rageBot[RcurrentCategory].minDamage, 0, 120, "%d");
                        config->rageBot[RcurrentCategory].minDamage = std::clamp(config->rageBot[RcurrentCategory].minDamage, 0, 120);

                        ImGui::PushID("Damage override Key");
                        ImGui::hotkey2(skCrypt("Damage override Key"), config->minDamageOverrideKey);
                        ImGui::PopID();

                        if (!config->minDamageOverrideKey.isOff())
                            ImGui::SliderInt(skCrypt("Damage-override"), &config->rageBot[RcurrentCategory].minDamageOverride, 0, 120, "%d");

                        config->rageBot[RcurrentCategory].minDamageOverride = std::clamp(config->rageBot[RcurrentCategory].minDamageOverride, 0, 120);

                        ImGui::Checkbox(skCrypt("Prefer body-aim"), &config->rageBot[RcurrentCategory].preferBodyAim);
                        ImGui::Checkbox(skCrypt("Prefer safe-points"), &config->rageBot[RcurrentCategory].preferSafePoints);

                        ImGui::Checkbox(skCrypt("Disable multi-point if low FPS"), &config->ragebot.disableMultipointIfLowFPS);
                        ImGui::PopItemWidth();
                    }
                    ImGui::EndGroup();
                }
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##antiaim"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ...
                    ImGui::PushItemWidth(245.f);
                    ImGui::Checkbox(skCrypt("Enabled"), &config->condAA.global);
                    ImGui::SameLine();
                    ...
                    if (ImGui::BeginPopup(""))
                    {
                        static const char* brekears[]{ "Static legs in air", "Zero pitch on land", "No animations", "Walk in air", "Move lean"};
                        static bool brek[ARRAYSIZE(brekears)] = { false, false, false, false, false };
                        static std::string previewvalue = "";
                        bool once = false;

                        for (size_t i = 0; i < ARRAYSIZE(brek); i++)
                        {
                            brek[i] = (config->condAA.animBreakers & 1 << i) == 1 << i;
                        }

                        if (ImGui::BeginCombo(skCrypt("Animation breakers"), previewvalue.c_str()))
                        {
                            previewvalue = "";
                            for (size_t i = 0; i < ARRAYSIZE(brekears); i++)
                            {
                                ImGui::Selectable(brekears[i], &brek[i], ImGuiSelectableFlags_::ImGuiSelectableFlags_DontClosePopups);
                            }
                        ...
                        for (size_t i = 0; i < ARRAYSIZE(brekears); i++)
                        {
                            ...
                            if (brek[i])
                            {
                                previewvalue += previewvalue.size() ? std::string(", ") + brekears[i] : brekears[i];
                    ...
                    ImGui::Combo(skCrypt("Pitch"), &config->rageAntiAim[current_category].pitch, skCrypt("Off\0Down\0Zero\0Up\0"));
                    ImGui::Combo(skCrypt("Yaw base"), reinterpret_cast<int*>(&config->rageAntiAim[current_category].yawBase), skCrypt("Off\0Forward\0Backward\0"));
                    ...
                    if (config->rageAntiAim[current_category].yawModifier == 1 || config->rageAntiAim[current_category].yawModifier == 2) //Jitter
                        ImGui::SliderInt(skCrypt("Jitter range"), &config->rageAntiAim[current_category].jitterRange, -180, 180, "%d");
                    ...
                    if (config->rageAntiAim[current_category].yawModifier == 4 || config->rageAntiAim[current_category].yawModifier == 5)
                        ImGui::SliderInt(skCrypt("Range"), &config->rageAntiAim[current_category].jitterRange, -180, 180, "%d");
                    ...
                    ImGui::Checkbox(skCrypt("Anti-Backstab"), &config->condAA.antiStab);
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##antiaim2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Other"));
                    ImGui::PopFont();

                    ImGui::Spacing();

                    ImGui::PushItemWidth(245.f);
                    ImGui::Checkbox(skCrypt("Enabled Fake"), &config->rageAntiAim[current_category].desync);
                    ImGui::SameLine();
                    ImGui::hotkey2(skCrypt("Invert Key"), config->invert, 80.0f);

                    // ImGui::Combo(skCrypt("Fake-lag"), &config->fakelag.mode, skCrypt("Static\0Adaptive\0Random\0"));
                    ImGui::SliderInt(skCrypt("Fake-lag Limit"), &config->fakelag.limit, 1, 15, "%d");
                    ...
                    ImGui::Combo(skCrypt("Peek Mode"), &config->rageAntiAim[current_category].peekMode, skCrypt("Off\0Peek real\0Peek fake\0Jitter\0"));
                    ImGui::Combo(skCrypt("LBY Mode"), &config->rageAntiAim[current_category].lbyMode, skCrypt("Normal\0Opposite\0Sway\0"));
                    ...
                    ImGui::Checkbox(skCrypt("Fake-duck"), &config->misc.fakeduck);
                    ImGui::SameLine();

                    ImGui::PushID(skCrypt("Fake-duck Key"));
                    ImGui::hotkey2("", config->misc.fakeduckKey);
                    ImGui::PopID();

                    ImGui::Checkbox(skCrypt("Slow-walk"), &config->misc.slowwalk);
                    ImGui::SameLine();

                    ImGui::PushID(skCrypt("Slow-walk Key"));
                    ImGui::hotkey2("", config->misc.slowwalkKey);
                    ImGui::PopID();

                    if (config->misc.slowwalk)
                        config->misc.slowwalkAmnt = 45;
                    ImGui::PopItemWidth();
                }
                ImGui::EndGroup();
            }
        ...
        if (selected == 2)
        {
            ...
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##visuals"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("World"));
                    ImGui::PopFont();
                    ...
                    ImGui::SliderFloat(skCrypt("Aspect Ratio"), &config->misc.aspectratio, 0.0f, 5.0f, skCrypt("%.2f"));
                    ImGui::SliderInt(skCrypt("Fov"), &config->visuals.fov, -90, 90, skCrypt("%d"));
                    ...
                    ImGui::Checkbox(skCrypt("World changer"), &wrld_enabled);
                    ImGui::SameLine();
                    ImGui::PushID(skCrypt("World changer"));
                    ...
                    if (ImGui::BeginPopup(""))
                    {
                        ...
                        ImGui::PushID(13);
                        ImGui::SliderInt(skCrypt("Asus walls"), &config->visuals.asusWalls, 0, 100, skCrypt("%d"));
                        ImGui::PopID();
                        ImGui::PushID(14);
                        ImGui::SliderInt(skCrypt("Asus props"), &config->visuals.asusProps, 0, 100, skCrypt("%d"));
                        ImGui::PopID();
                        ...
                        if (ImGui::BeginPopup(""))
                        {
                            ImGui::SliderInt(skCrypt("X rotation"), &config->visuals.shadowsChanger.x, 0, 360, skCrypt("%d"));
                            ImGui::SliderInt(skCrypt("Y rotation"), &config->visuals.shadowsChanger.y, 0, 360, skCrypt("%d"));
                            ImGui::EndPopup();
                        }
                        ImGui::PopID();
                        ...
                        if (config->visuals.inverseRagdollGravity)
                            ImGui::SliderInt(skCrypt("Gravity amount"), &config->misc.ragdollGravity, -1500, 1500, "%d");
                        ImGui::PopID();
                    ...
                    ImGui::Text(skCrypt("Scope settings"));
                    ...
                    if (config->visuals.scope.type == 2)
                    {
                        ...
                        if (ImGui::BeginPopup(""))
                        {
                            ...
                            ImGui::Checkbox(skCrypt("Remove top line"), &config->visuals.scope.removeTop);
                            ImGui::SameLine();
                            ImGui::Checkbox(skCrypt("Remove bottom line"), &config->visuals.scope.removeBottom);
                            ImGui::Checkbox(skCrypt("Remove left line"), &config->visuals.scope.removeLeft);
                            ImGui::SameLine();
                            ImGui::Checkbox(skCrypt("Remove right line"), &config->visuals.scope.removeRight);
                            ImGui::EndPopup();
                        }
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##visuals2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ...
                    {
                        ImGui::SameLine();
                        ImGui::PushID("?1'thirdperson'1?tf is u looking in hex for 'ecvator");
                        ...
                        {
                            ImGui::SliderInt(skCrypt("Thirdperson distance"), &config->visuals.thirdperson.distance, 50, 300, skCrypt("%d"));
                            ImGui::SliderFloat(skCrypt("Transparency in scope"), &config->visuals.thirdperson.thirdpersonTransparency, 0.f, 100.f, "%.1f");

                            ImGui::Checkbox(skCrypt("While dead"), &config->visuals.thirdperson.whileDead);
                            ImGui::Checkbox(skCrypt("Disable on grenade"), &config->visuals.thirdperson.disableOnGrenade);
                            ImGui::EndPopup();
                        }
                        ImGui::PopID();
                    }
                    ...
                    if (ImGui::BeginPopup(("htiqwi")))
                    {
                        ...
                        ImGui::SliderFloat(skCrypt("Hit marker time"), &config->visuals.hitMarkerTime, 0.1f, 1.5f, "%.2fs");
                        ImGui::EndPopup();
                    }
                    ImGui::PopID();
                    ...
                    ImGui::SliderFloat(skCrypt("Bullet Impacts time"), &config->visuals.bulletImpactsTime, 0.1f, 5.0f, "%.2fs");
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##players"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Esp"));
                    ImGui::PopFont();
                    ...
                    ImGui::Combo("for", &currentCategory, skCrypt("Enemies\0Allies\0Weapons\0Projectiles\0"), -1, false);
                    ...
                    if (currentCategory <= 1)
                    {
                        ...
                        ImGui::Checkbox(skCrypt("Dlights"), &pCfg2.enabled);
                        ...
                        if (ImGui::BeginPopup(""))
                        {
                            ImGuiCustom::colorPicker(skCrypt("Color"), pCfg2.asColor3);
                            ImGui::SliderFloat(skCrypt("Radius"), &pCfg2.raduis, 5.f, 150.f, "%.2f");
                            ImGui::SliderInt(skCrypt("Exponent"), &pCfg2.exponent, 1, 20, "%d");
                            ImGui::EndPopup();
                        }
                        ImGui::PopID();
                        ...
                        if (ImGui::BeginPopup(""))
                        {
                            ImGui::Combo(skCrypt("Health bar type"), &pCfg.health.style, skCrypt("Solid\0Gradient\0"));
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##players2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Chams"));
                    ImGui::PopFont();
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##misc"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Misc"));
                    ImGui::PopFont();
                    ...
                    ImGui::Combo(skCrypt("Changer"), &config->misc.clantag_type, skCrypt("ecvator\0ecvator modern\0estr'k\0"));

                    ImGui::Checkbox(skCrypt("Auto reload"), &config->misc.autoReload);
                    ImGui::Checkbox(skCrypt("Fast switch"), &config->misc.autoPistol);
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##misc2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Movement"));
                    ImGui::PopFont();

                    ImGui::Spacing();
                    ImGui::PushItemWidth(245.f);

                    ImGui::Checkbox(skCrypt("Infinity duck"), &config->misc.fastDuck);
                    ...
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Info"));
                    ImGui::PopFont();
                    ...
                    ImGui::Combo(skCrypt("Hit Sound"), &config->misc.hitSound, skCrypt("None\0Defualt\0Metallic\0Skeet\0"));

                    ImGui::Checkbox(skCrypt("Watermark"), &config->misc.wm.enabled);
                    ImGui::SameLine();
                    ImGui::PushID(skCrypt("Watermark"));
                    ...
                    if (ImGui::BeginPopup(""))
                    {
                        static const char* toShow[]{ "Double tap","On Shot","Direction Anti-aim","Damage override","Hitchance override","Manual anti-aim","Freestand","Force baim","Auto peek","Fake duck" };
                        static bool toShow1[ARRAYSIZE(toShow)] = { false, false, false, false, false, false, false, false, false, };
                        static std::string previewvalue = "";
                        bool once = false;
                    ...
                    if (ImGui::BeginPopup(""))
                    {
                        ...
                        // ImGuiCustom::colorPicker(skCrypt("Color logs"), config->misc.logger.color, &config->misc.logger.color);
        ...
        if (selected == 5)
        {
            ...
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##skins"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Skins"));
                    ImGui::PopFont();
                    ...
                    ImGui::Separator();
                    ImGui::Text(skCrypt("Seed"));
                    ImGui::InputInt(skCrypt(""), &selected_entry.seed);
                    ImGui::Text(skCrypt("StatTrak"));
                    ImGui::InputInt(skCrypt(""), &selected_entry.stat_trak);
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##skins2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ...
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Setup"));
                    ImGui::PopFont();
                    ...
                    if (ImGui::Button1("Update", { 250.0f, 25.0f }))
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##settings"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Configs"));
                    ImGui::PopFont();
                    ...
                    if (ImGui::ListBox("", &currentConfig, [](void* data, int idx, const char** out_text)
                        ...
                        if (ImGui::Button1(skCrypt("Open config directory"), ImVec2{ 250.0f, 25.0f }))
                            config->openConfigDir();

                        if (ImGui::Button1(skCrypt("Create config"), ImVec2{ 250.0f, 25.0f }))
                        {
                            config->add(buffer.c_str());
                            ImGui::InsertNotification({ ImGuiToastType_Success, 3000, "Configs loaded! Success" });
                        }

                        if (currentConfig != -1)
                        {
                            if (ImGui::Button1(skCrypt("Load selected"), ImVec2{ 250.0f, 25.0f }))
                            {
                            ...
                            if (ImGui::BeginPopup(skCrypt("##reallySave")))
                            {
                                ...
                                if (ImGui::Button1(skCrypt("Yes"), { 45.0f, 0.0f }))
                                {
                                    ...
                                    ImGui::InsertNotification({ ImGuiToastType_Success, 3000, "Configs saved! Success" });
                                }
                                ImGui::EndPopup();
                            }
                            ImGui::PopID();
                            ...
                            if (ImGui::Button1(skCrypt("Delete selected"), ImVec2{ 250.0f, 25.0f }))
                            ...
                            if (ImGui::BeginPopup(skCrypt("##reallyDelete")))
                            {
                                ...
                                if (ImGui::Button1(skCrypt("Yes"), { 45.0f, 0.0f }))
                                {
                                    ...
                                    ImGui::InsertNotification({ ImGuiToastType_Success, 3000, "Configs delete! Success" });
                                }
                                ImGui::EndPopup();
                            }
                            ImGui::PopID();
                        ...
                        if (ImGui::Button1(skCrypt("Reset selected"), ImVec2{ 250.0f, 25.0f }))
                        ...
                        if (ImGui::BeginPopup(skCrypt("Config to reset")))
                        {
                            ...
                            for (int i = 0; i < IM_ARRAYSIZE(names); i++)
                            {
                                ...
                                if (ImGui::Selectable(names[i]))
                                {
                                    ...
                                    case 9: config->streamProofESP = { }; config->esp = { }; break;
                                ...
                                ImGui::InsertNotification({ ImGuiToastType_Success, 3000, "Configs reset! Success" });
                            }
                            ImGui::EndPopup();
                        }
                        ImGui::PopItemWidth();
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##player_list"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Player list"));
                    ImGui::PopFont();
                    ...
                    ImGui::TextColored(ImColor(Helpers::calculateColor(config->menu.accentColor)), skCrypt("Coming soon..."));
                    ImGui::PopFont();
                }
                ImGui::EndGroup();
            }
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##scripts"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Scripts"));
                    ImGui::PopFont();
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##scripts2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Tab A&B"));
                    ImGui::PopFont();
        ...
        {
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##interface"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Interface"));
                    ImGui::PopFont();
                    ...
                    ImGuiCustom::colorPicker(skCrypt("Accent windows bg"), config->menu.accentColorWindowsBG);
                    ImGuiCustom::colorPicker(skCrypt("Accent child bg"), config->menu.accentColorChildBG);
                    ImGuiCustom::colorPicker(skCrypt("Accent popup bg"), config->menu.accentColorPopupBG);

                    ImGui::Checkbox(skCrypt("Outline border"), &config->menu.outline_menu);
            ...
            ImGui::EndChild();
            ImGui::SameLine();
            ImGui::BeginChild(skCrypt("##interface2"), ImVec2{ 280, 0 }, true);
            {
                ImGui::SetCursorPos({ 10, 10 });
                ImGui::BeginGroup();
                {
                    ImGui::PushFont(fonts.tahoma16);
                    ImGui::TextColored(ImColor(225, 225, 225), skCrypt("Soon.."));
                    ImGui::PopFont();
