...
public:
    ...
    {
        static auto m_uInvalidatePhysics = memory->findPattern(CLIENT_DLL, skCrypt("55 8B EC 83 E4 F8 83 EC 0C 53 8B 5D 08 8B C3 56 83 E0 04").decrypt(), true);
        reinterpret_cast <void(__thiscall*)(void*, int)> (m_uInvalidatePhysics)(this, change_flags);
    }

    // https://github.com/pmrowla/hl2sdk-csgo/blob/49e950f3eb820d88825f75e40f56b3e64790920a/game/server/baseentity.cpp#L6569
    // @ptr: we should set right abs velocity how to game does.
    void SetAbsVelocity(Vector& vecVelocity)
    {
        // The abs velocity won't be dirty since we're setting it here
        // All children are invalid, but we are not
        // 0x4 -> VELOCITY_CHANGED
        //InvalidatePhysicsRecursive(4);
        getEFlags() &= ~0x1000;

        getAbsVelocity() = vecVelocity;
    }
    ...
    bool setupBonesSkeet(Entity* player, matrix3x4* bone, int max_bones, int mask, float currentTime)
    {
        const int backup_ik = *(int*)(uintptr_t(player) + (0x1c));
        const int backup_client_effects = *(int*)(uintptr_t(player) + 0x68);
        const int backup_effects = player->getEffects(); // ( "CBaseEntity", "m_fEffects" )
        ...
        const Vector backup_abs_origin = player->getAbsOrigin(); // 10 index from c_base_ent virtual table..
        ...
        const auto jiggleBones = interfaces->cvar->findVar(skCrypt("r_jiggle_bones"));
        const int backup_jiggle_bones = jiggleBones->getInt();
        jiggleBones->setValue(0);

        *(int*)(uintptr_t(player) + (0x1c)) = 0;
        ...
        *(int*)(uintptr_t(player) + 0x68) |= 2;
        ...
        // fix model sway...
        float v105 = -1;
        if (player->getAnimationLayersCount() >= 12)
        {
            v105 = player->animOverlays()[12].weight;
            player->animOverlays()[12].weight = 0;
        }

        // set un-interpolated origin.
        if (player != localPlayer.get())
            memory->setAbsOrigin(this, backup_abs_origin);

        // set time too.
        currentTime = 0;
        if (player != localPlayer.get())
            currentTime = player->simulationTime();
        else
            currentTime = memory->globalVars->serverTime();

        // es0 shizo.
        const int backup_frame_cnt = memory->globalVars->framecount;
        memory->globalVars->framecount = -999; // bypass should_skip_anim_frame.
        ...
        // build bones.
        bool premium_geng = VirtualMethod::call<bool, 13>(this + sizeof(uintptr_t), bone, max_bones, mask, currentTime);

        // restore frame cnt.
        memory->globalVars->framecount = backup_frame_cnt;
        ...
        // restore prev.
        if (player->getAnimationLayersCount() >= 12 && v105 >= 0)
            player->animOverlays()[12].weight = v105;

        // ..........
        *(int*)(uintptr_t(player) + (0x1c)) = backup_ik;
        ...
        *(int*)(uintptr_t(player) + 0xa28) = backup_anim_lod_flags;
        ...
        jiggleBones->setValue(backup_jiggle_bones);
    ...
    void runThink() noexcept
    {
        ...
        if (nextThinkTick != -1 && nextThinkTick > 0 && nextThinkTick <= tickBase())
        {
    ...
    NETVAR(body, "CBaseAnimating", "m_nBody", int)
        ...
        NETVAR(m_flPostponeFireReadyTime, "CWeaponCSBase", "m_flPostponeFireReadyTime", float)
        NETVAR(is_walking, "CCSPlayer", "m_bIsWalking", bool);
