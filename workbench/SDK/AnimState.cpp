...
void AnimState::setupVelocity() noexcept
{
    ...
    if (getLayerActivity(ANIMATION_LAYER_ADJUST) == ACT_CSGO_IDLE_ADJUST_STOPPEDMOVING || getLayerActivity(ANIMATION_LAYER_ADJUST) == ACT_CSGO_IDLE_TURN_BALANCEADJUST)
    {
...
void AnimState::setupMovement() noexcept
{
    ...
    onGround = (entity->flags() & FL_ONGROUND);
...
void AnimState::doAnimationEvent(int animationEvent) noexcept
{
    ...
    case PLAYERANIMEVENT_DEPLOY:
    {
        ...
        if (currentWeapon && getLayerActivity(ANIMATION_LAYER_WEAPON_ACTION) == ACT_CSGO_DEPLOY && getLayerCycle(ANIMATION_LAYER_WEAPON_ACTION) < 0.15f/*CSGO_ANIM_DEPLOY_RATELIMIT*/)
        {
