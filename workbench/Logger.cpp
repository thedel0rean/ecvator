...
void Logger::getEvent(GameEvent* event) noexcept
{
    ...
    {
        const int hurt = interfaces->engine->getPlayerForUserID(event->getInt(skCrypt("userid")));
        const int attack = interfaces->engine->getPlayerForUserID(event->getInt(skCrypt("attacker")));
    ...
    case fnv::hash("bomb_planted"): 
    {
        ...
        const int idx = interfaces->engine->getPlayerForUserID(event->getInt(c_xor("userid")));
        ...
        const std::string site = GameData::plantedC4().bombsite ? "A" : "B";
    ...
    case fnv::hash("item_purchase"):
    {
        ...
        auto userId = interfaces->engine->getPlayerForUserID(event->getInt(c_xor("userid")));
        if (!userId)
            break;
        ...
        PlayerInfo ppi = player->get_info();
        if (!interfaces->engine->getPlayerInfo(userId, ppi))
            break;

        std::string weapon = event->getString(c_xor("weapon"));

        std::ostringstream ss; ss << ppi.name << " bought " << weapon;
    ...
    case fnv::hash("bomb_beginplant"):
    {
        ...
        log.text = ppi.name + std::string(skCrypt(" is planting the bomb"));
        break;
    }
    case fnv::hash("bomb_begindefuse"):
    {
        ...
        log.text = ppi.name + std::string(skCrypt(" is defusing the bomb"));
        break;
    }
    default:
        return;
...
void Logger::console() noexcept
{
    ...
    for (auto log : logs)
    {
        Helpers::logConsole(c_xor("Ecvator | "), color);
