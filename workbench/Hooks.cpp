...
static void __stdcall paintTraverse(unsigned int panel, bool forceRepaint, bool allowForce) noexcept
{
    ...
    if (set_console)
    {
        ...
        interfaces->cvar->findVar(skCrypt("developer"))->setValue(FALSE); //-V807
        interfaces->cvar->findVar(skCrypt("con_filter_enable"))->setValue(TRUE);
        interfaces->cvar->findVar(skCrypt("con_filter_text"))->setValue(skCrypt(""));

        interfaces->engine->clientCmdUnrestricted("clear");
        interfaces->engine->clientCmdUnrestricted("sv_lan 1");
    }
    ...
    if (reload_fonts())
    {
        ...
        hooks->nameEsp = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->nameEsp, c_xor("Verdana"), 12, FW_MEDIUM, 0, 0, fontflag_antialias | fontflag_dropshadow, 0x20, 0xa69f);
        ...
        hooks->loggerFont = interfaces->surface->createFont(); interfaces->surface->setFontGlyphSet(hooks->loggerFont, c_xor("Consolas"), 10, 400, 0, 0, 0x200 | 0x010 | 0x080, 0, 0);
...
static bool __stdcall createMove(float inputSampleTime, UserCmd* cmd, bool& sendPacket) noexcept
{
    ...
    {
        sendPacket = Tickbase::sendPacket;
        ...
        if ((currentViewAngles != cmd->viewangles || cmd->forwardmove != currentCmd.forwardmove || cmd->sidemove != currentCmd.sidemove) && (cmd->sidemove != 0 || cmd->forwardmove != 0))
...
{
    if (std::uintptr_t(_ReturnAddress()) == memory->cameraThink && (config->visuals.thirdperson.enable))
        return true;
...
static void __stdcall frameStageNotify(FrameStage stage) noexcept
{
    ...
    return hooks->client.callOriginal<void, 37>(stage);
}
...
static void __stdcall setDrawColor(int r, int g, int b, int a) noexcept
{
    if (config->visuals.noScopeOverlay && (std::uintptr_t(_ReturnAddress()) == memory->scopeDust || std::uintptr_t(_ReturnAddress()) == memory->scopeArc))
        a = 0;
    if (localPlayer && (config->visuals.scope.type == 2 || config->visuals.scope.type == 1 || config->visuals.scope.type == 3) && (std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensClear || std::uintptr_t(_ReturnAddress()) == memory->staticScopeLensBlurry))
        a = 0;
...
static void __stdcall overrideView(ViewSetup* setup) noexcept
{
    ...
    {
        //clip players
        bool thirdperson = config->visuals.thirdperson.enable && config->visuals.thirdperson.key.isActive();
        float distance = 0.f;
        ...
        else if (memory->input->isCameraInThirdPerson && !thirdperson)
        {
            distance = Helpers::approachValueSmooth(distance, 0.0f, memory->globalVars->frametime * 7.f);
            angles.z = distance * tr.fraction;
            memory->input->cameraOffset = angles;
            if (distance < 30.f)
...
static int __fastcall dispatchSound(SoundInfo& soundInfo) noexcept
{
    if (const char* soundName = interfaces->soundEmitter->getSoundName(soundInfo.soundIndex)) {
        soundInfo.volume = std::clamp(soundInfo.volume, 50.0f, 100.0f);
    }
...
{
    static auto original_fn = hooks->buildTransformations.getOriginal<void>(ecx, FilePath);

    if (strstr(FilePath, ("facemask_battlemask.mdl")))
        sprintf(FilePath, ("models/player/holiday/facemasks/facemask_dallas.mdl"));

    return original_fn(ecx, edx, FilePath);
}
...
static void __fastcall hkRunCommand(void* ecx, void* edx, Entity* player, UserCmd* cmd, MoveHelper* movehelper)
{
    using PredictionHookFn = void(__fastcall*)(void*, Entity*, UserCmd*, MoveHelper*);
    static auto original = (PredictionHookFn)hooks->PredictionHook.getDetour();

    if (!player || !cmd || player != localPlayer.get())
        return original(ecx, player, cmd, movehelper);

    const float backup_velocity_modifier = player->velocityModifier();

    if (cmd->tickCount > memory->globalVars->tickCount * 2)
    {
        cmd->hasbeenpredicted = true;
        return;
    }

    original(ecx, player, cmd, movehelper);
    player->velocityModifier() = backup_velocity_modifier;
...
{
    // static auto original = hooks->client.getOriginal<bool, 24, int, bufferWrite*, int, int, bool>(slot, buffer, from, to, is_new_cmd);
...
void Studio_SolveIK(Entity* player, int thigh, int knee, int foot, Vector& target_foot, matrix3x4* bone_to_world) {
    //hooks->studioSolveIK.getOriginal<bool* (__thiscall)(Entity*, int, int, int, Vector&, matrix3x4*)>();
}

void clamp_bones_in_bbox(Entity* player, matrix3x4* matrix, int mask, float curtime)
{
    ...
    //auto bbox_max = collideable->obbMaxs().z + collision_origin;

    //auto last_update_increment = curtime - player->collisionChangeTime();
    ...
    //__patterns->weapon_shootpos.cast<float* (__thiscall*)(void*, Vector*)>()(player, &pos);

    //Helpers::AngleVectors(player->eyeAngles(), forward, right, up);
    ...
    //auto clamped_bones_new = Vector{ new_bone_pos.x - pos.x, new_bone_pos.y - pos.y, 0.f };
...
void Hooks::install() noexcept
{
    ...
    **reinterpret_cast<decltype(reset)***>(memory->reset) = reset;
    ...
    //PredictionHook.init(memory->clientMode);
    //PredictionHook.hookAt(19, hkRunCommand);
    ...
    mdlcache.detour(10, hooked_findmdl);
