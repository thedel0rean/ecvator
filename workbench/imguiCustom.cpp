...
void ToggleeButton(const char* str_id, bool* v)
{
...
bool Checkbox(const char* label, bool* v)
{
    ...
    if (*v)
    {
        ...
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, ImGui::GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
...
void ImGuiCustom::checkbox(const char* label, bool* v)
{
    Checkbox(label, v);
}
...
void ImGui::hotkey2(const char* label, KeyBind& key, float samelineOffset, const ImVec2& size, bool border) noexcept
{
    ...
    else
    {
        if (key.keyMode == KeyMode::Always)
        {
            ...
            ImGui::ButtonEx("On", {}, ImGuiButtonFlags_Disabled, border);
            ImGui::PopStyleColor();
        }
        else if (key.keyMode == KeyMode::Off)
        {
            ImGui::PushStyleColor(ImGuiCol_Text, ImGui::GetColorU32(ImGuiCol_TextDisabled));
            ImGui::ButtonEx("Off", {}, ImGuiButtonFlags_Disabled, border);
            ImGui::PopStyleColor();
        }
        else if (key.isSet())
        {
