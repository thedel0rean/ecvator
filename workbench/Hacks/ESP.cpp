...
void FilledQuad(const Vector2D& p1, const Vector2D& p2, const Vector2D& p3, const Vector2D& p4, const Color& color) {
    ...
    interfaces->surface->drawTexturedPolygon(4, vertices);
}

void DrawArc(const Vector2D& center, float radius, float progress, const Color& color, float thickness = 4, int num_segments = 64) {

    interfaces->surface->setDrawColor(255, 255, 255);

    if (radius == 0.0f) 
    {
        interfaces->surface->drawFilledRect(center.x, center.y, center.x + 1, center.y + 1);
        return;
    }

    float inner_radius = max(0.0f, radius - thickness);
    float outer_radius = radius;

    num_segments = max(num_segments, 3);

    float angle = progress * pi;
    float angle_step = angle / num_segments;

    for (int i = 0; i <= num_segments; i++) 
    {
        float current = i * angle_step - pi / 2;
        float next = (i + 1) * angle_step - pi / 2;

        Vector2D inner[2] = {
            { center.x + cos(current) * inner_radius, center.y + sin(current) * inner_radius },
            { center.x + cos(next) * inner_radius, center.y + sin(next) * inner_radius}
        };

        Vector2D outer[2] = {
            { center.x + cos(current) * outer_radius, center.y + sin(current) * outer_radius },
            { center.x + cos(next) * outer_radius, center.y + sin(next) * outer_radius}
        };

        FilledQuad(inner[0], outer[0], outer[1], inner[1], color);
    }
}

void warning::draw_circle(int x, int y, int points, int radius, Color color)
{
...
void draw_arc(const Vector2D& center, float radius, float progress, const Color& color, float thickness = 4, int num_segments = 64)
{
    DrawArc(center, radius, progress, color, thickness, num_segments);
};
...
static void healthBar(const PlayerData& player, const BoundingBox& bbox, Config::NewESP::Player& config)
{
    if (config.health.enabled)
    {
        ...
        {
            interfaces->surface->setDrawColor(min(255 - ((max(0, min(player.health, 100)) * 5.10f)), 255.f), min(max(0, min(player.health, 100)) * 2.55f, 255.f), 0);
            interfaces->surface->drawFilledRect(drawPositionLeft - 2, bbox.y1 - abs(bbox.y1 - bbox.y0) * (100 - player.health) / 100, drawPositionLeft, bbox.y1);
        }
...
static void renderProjectileESP(const ProjectileData& entity, const char* name, const char* icon, Config::NewESP::Projectiles& config)
{
    ...
    if (!entity.exploded)
    {
        renderBox(bbox, config.box);
        renderName(bbox, config.name, name, hooks->smallFonts, false);
        // warning::draw_circle(bbox.x0, bbox.y0, 460, 15, Color(0, 0, 0));
    }
}
