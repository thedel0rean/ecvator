    ...
    for (size_t i = 0; i < molotovs.size(); i++)
    {
        ...
        if (time >= 0.0f) 
        {
            if (Helpers::worldToScreen(molotov.pos, pos))
            {
                ...
                if (fraction == 1.0f)
                {
                    //drawList->AddCircle(pos, radius, Helpers::calculateColor(config->visuals.molotovTimerTimer), 40, /*2.0f +*/ 2.65f);
                }
                else
                {
                    constexpr float pi = std::numbers::pi_v<float>;
...
void Visuals::thirdperson() noexcept
{
    ...
    static auto collision = interfaces->cvar->findVar(c_xor("cam_collision")); collision->setValue(1);
    static auto idealyaw = interfaces->cvar->findVar(c_xor("cam_idealyaw")); idealyaw->setValue(0);
    static auto idealpitch = interfaces->cvar->findVar(c_xor("cam_idealpitch")); idealpitch->setValue(0);
...
void Visuals::hitEffect(GameEvent* event) noexcept
{
    if (config->visuals.hitEffect && localPlayer) 
    {
        ...
        if (event && interfaces->engine->getPlayerForUserID(event->getInt(c_xor("attacker"))) == localPlayer->index()) 
        {
...
{
    if (!config->visuals.onHitHitbox.enabled) 
        return;

    if (config->visuals.onHitHitbox.duration <= 0.f) 
        return;
    ...
    if (localPlayer->getUserId() != attacker->getUserId() || localPlayer->getUserId() == userID->getUserId()) 
        return;

    StudioHdr* hdr = interfaces->modelInfo->getStudioModel(userID->getModel());
    StudioHitboxSet* set = hdr->getHitboxSet(0);
    matrix3x4* matrix = userID->getBoneCache().memory;
    ...
    for (int i = 0; i < set->numHitboxes; i++) 
    {
        ...
        VectorTransform_Wrapper(vectorRotate(hitbox->bbMin, hitbox->offsetOrientation), matrix[hitbox->bone], vMin);
        VectorTransform_Wrapper(vectorRotate(hitbox->bbMax, hitbox->offsetOrientation), matrix[hitbox->bone], vMax);

        interfaces->debugOverlay->capsuleOverlay(vMin, vMax, hitbox->capsuleRadius, r, g, b, a, d, 0, 1);
    }
}
...
{
    return config->visuals.noWeapons && std::strstr(modelName, skCrypt("models/weapons/v_")) && !std::strstr(modelName, skCrypt("arms")) && !std::strstr(modelName, skCrypt("tablet")) && !std::strstr(modelName, skCrypt("parachute")) && !std::strstr(modelName, skCrypt("fists"));
}
...
void Visuals::bulletTracer(GameEvent* event) noexcept
{
    ...
    case fnv::hash("bullet_impact"):
    {
        ...
        beamInfo.width = 0.55f;
        beamInfo.endWidth = 0.55f;
        beamInfo.flags = 0x40;
