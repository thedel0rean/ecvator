...
void Resolver::SetModeResolver(Animations::Players player, Entity* entity)
{
...
//void Resolver::resolve_stand(Animations::Players player, Animations::Players prev_player, Entity* entity, float angle)
//{
//    
//    
//
...
void Resolver::processMissedShots() noexcept
{
    ...
    for (int hitbox = 0; hitbox < Hitboxes::Max; hitbox++)
    {
        ...
        std::string missedJitter = std::string(skCrypt("missed shot on ")) + entity->getPlayerName() + std::string(skCrypt(" due to jitter resolver"));
    ...
    if (!resolverMissed && snapshot.gotImpact)
        Logger::addLog(std::string(skCrypt("missed shot on ")) + entity->getPlayerName() + std::string(skCrypt(" due to prediction error")));

    if (!resolverMissed)
        Logger::addLog(std::string(skCrypt("missed shot on ")) + entity->getPlayerName() + std::string(skCrypt(" due to spread")));
}
