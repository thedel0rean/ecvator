...
void drawParticles(Vector position) noexcept
{
	float step = 3.141592654f * 2.0f;
	ImVec2 end2d{}, start2d{};
	Vector lastPos{};
	for (float a = -step; a < 3.141592654f * 2.0f; a += step) {
		Vector start{ cosf(a) + position.x, sinf(a) + position.y, position.z };
		...
		interfaces->m_Effects->MuzzleFlash(*endPos.data(), Vector(0, 0, 100), 1.4, 1);
	}
}
...
void GrenadePrediction::run(UserCmd* cmd) noexcept
{
	...
	if (itemDefinition != WeaponId::SmokeGrenade && itemDefinition != WeaponId::Decoy && itemDefinition != WeaponId::Molotov && itemDefinition != WeaponId::IncGrenade && itemDefinition != WeaponId::Flashbang && itemDefinition != WeaponId::HeGrenade)
	{
		renderMutex.unlock();
		return;
	}
